/*
	...
	Параметры:
		...
		1. - места что блокируются 
			"driver"
			"gunner"
			"commander" - блокировка места командира - разрешено только для permittedCommanderСlasses
			"crew" - блокировка всей техники (вкл. пассажиров) разрешено только для permittedCrewСlasses
...
#define allCrew (typeOf _unitToCheck) in permittedCrewСlasses
#define isCrew (_vehicle iskindof "landvehicle" && (toLower(gettext Cfgman) find "crew" > -1 || toLower(gettext Cfgman) find "driver" > -1 || ((typeOf _unitToCheck) in permittedCommanderСlasses)) || (_vehicle iskindof "air" && toLower(gettext Cfgman) find "pilot" > -1) || allCrew )
