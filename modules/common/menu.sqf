...
_isTryUnlockAvailable = {
	...
	(((_target distance player) < 5) && (_target isKindOf "LandVehicle" || _target isKindOf "Helicopter" || _target isKindOf "Plane" || _target isKindOf "Ship" || _target isKindOf "StaticWeapon") && (locked(_target) > 1))
};
_tryUnlock = {
	_target = cursortarget;
	_max = switch true do {
