...
[localize "STR_DBS_SIMBuildingMode", 0,
	{[] call compile preprocessFile (_PREFIX_ + "_act_switch_buildingmode.sqf")},
	_serp_menu_cond1, _serp_menu_cond1] call SerP_addToMenu;		


_serp_menu_cond2 = {((!isNull player) && {GVAR(BuildSysEnabled) && (leader player == player)
				&& (isNil 'misBuildSys_EmptyListForAllowance')})};

	
[localize "STR_DBS_SIMAllowance", 0,
	{[] call compile preprocessFile (_PREFIX_ + "_act_giveallowance.sqf")},
	_serp_menu_cond2, _serp_menu_cond2] call SerP_addToMenu;		
...
//			_PREFIX_ + "_act_build.sqf", [], BASE_ACTION_PRIORITY + 10];
		...
		GVAR(MType) = player addAction [localize "STR_DBS_AMType",
			_PREFIX_ + "_act_forttype.sqf", [], BASE_ACTION_PRIORITY + 6];
		
		GVAR(MFlip) = player addAction [localize "STR_DBS_AMFlip",
			_PREFIX_ + "_act_flip_dir.sqf", [], BASE_ACTION_PRIORITY + 5];
